...
BOOL ReadMemory(HANDLE hDevice, uintptr_t address, PVOID buffer, SIZE_T size) {
	...
	// Отправка управляющего кода в драйвер с использованием DeviceIoControl
	status = DeviceIoControl(
		hDevice,                      // Устройство-драйвер
		IO_READ,            // Код управления
		&shared_request,                         // Входной буфер
		sizeof(shared_request),              // Размер входного буфера
		&shared_request,                         // Выходной буфер
		sizeof(shared_request),              // Размер выходного буфера
		NULL,               // Количество байт возвращено
		NULL                          // Переключение на синхронный ввод/вывод
	);
